...
export default function ClientChat() {
  ...
  function getOtherPartyId(conv: Conversation): string {
    return conv.type === 'job' ? (conv as JobConversation).otherPartyId : (conv as DmConversation).recipientId
  }
            ...
            <div className="flex-1 overflow-y-auto">
              {loadingConvs ? (
                ...
                <div className="p-8 text-center">
                  ...
                  <p className="text-sm text-gray-500">No conversations yet. Visit a driver profile to start messaging.</p>
                </div>
              ) : (
              ...
              <>
                ...
                {sendError && (
                  <div className="px-4 py-2 bg-red-50 border-t border-red-100 text-xs text-red-600 flex items-center justify-between">
                    <span>{sendError}</span>
                    <button type="button" onClick={() => setSendError('')} className="text-red-400 hover:text-red-600 ml-2">✕</button>
                  </div>
                )}
