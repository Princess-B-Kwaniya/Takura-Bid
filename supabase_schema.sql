...
-- ============================================================
-- IMPORTANT: Run this AFTER you already have Supabase Auth set up.
-- The users.user_id column references auth.users(id).
-- ============================================================
...
-- 1. USERS (already exists if you ran the earlier migration, but here for completeness)
...
CREATE TABLE IF NOT EXISTS loads (
  ...
  description       TEXT,
  requirements      TEXT[],          -- e.g. {'Flatbed Truck','Insurance Required'}
...
-- 6. NOTIFICATIONS (bonus — supports the SRS notification system)
CREATE TABLE IF NOT EXISTS notifications (
  notification_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id         UUID NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
  title           TEXT NOT NULL,
  body            TEXT NOT NULL,
  type            TEXT NOT NULL DEFAULT 'info',   -- 'info','bid','job','message','payment'
  read            BOOLEAN NOT NULL DEFAULT false,
  reference_id    TEXT,                           -- optional link to load/job/bid
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE INDEX IF NOT EXISTS idx_jobs_load         ON jobs(load_id);
...
-- Users: anyone authenticated can read, owners can update
...
-- Loads: everyone can read, clients can insert/update their own
...
-- Bids: everyone can read, drivers can insert their own
...
CREATE POLICY "Bid owners can update"
  ON bids FOR UPDATE TO authenticated USING (auth.uid() = driver_id);

-- Jobs: participants can read & update
...
-- Reviews: everyone can read, participants can insert
...
-- ============================================================
-- UPDATED_AT TRIGGER (auto-set updated_at on users)
-- ============================================================
...
-- ============================================================
-- BID COUNT TRIGGER (auto-update loads.bids_count)
-- ============================================================
...
-- ============================================================
-- ============================================================
-- SAMPLE DATA — 20 Users, Loads, Bids, Jobs, Messages
-- ============================================================
-- ============================================================
--
-- NOTE: These use hardcoded UUIDs so foreign keys work.
-- In production, user_id must match auth.users(id).
-- To use this sample data you have two options:
--   A) Temporarily DISABLE the FK to auth.users, insert, then re-enable.
--   B) Create 20 auth users first (via Supabase dashboard or Auth API)
--      and replace the UUIDs below with the real auth user IDs.
--
-- Below we use Option A for convenience:
-- ============================================================

-- Temporarily drop the FK constraint so we can insert demo users
...
-- Assigned loads (driver selected but not yet started)
...
-- In Bidding loads (open for bids)
...
VALUES
-- Bids on LOAD001 (In Transit — winning bid accepted)
...
-- Bids on LOAD002 (In Transit — winning bid accepted)
...
-- Pending / Active jobs (assigned but not started)
...
-- Conversation for JOB001: Tendai ↔ ABC Construction
...
-- Conversation for JOB002: James ↔ Zimbabwe Mining Corp
INSERT INTO messages (job_id, sender_id, recipient_id, content, read, sent_at) VALUES
...
-- Conversation for JOB004: Sarah ↔ Farm Fresh (completed job)
INSERT INTO messages (job_id, sender_id, recipient_id, content, read, sent_at) VALUES
...
-- Conversation for JOB005: Patricia ↔ Safari Lodge
INSERT INTO messages (job_id, sender_id, recipient_id, content, read, sent_at) VALUES
...
-- Conversation for JOB008: Patricia ↔ MediSupply (pending)
INSERT INTO messages (job_id, sender_id, recipient_id, content, read, sent_at) VALUES
...
-- ============================================================
-- RE-ADD THE FOREIGN KEY (after sample data is inserted)
-- ============================================================
-- Uncomment the line below ONLY if you want to restore the FK constraint.
-- In a demo/dev environment you can leave it off. In production, always keep it.
--
...
-- ============================================================
-- DONE! Your TakuraBid database is fully populated.
-- ============================================================
--
-- Summary:
...
--   • 30 bids across multiple loads
--   • 10 jobs (3 in-transit, 4 completed, 2 pending, 1 active)
--   • 23 messages across 5 conversation threads
